    ...
    <div class="divPokemon">
      ...
      >
        ...
        <div class="row" style="margin-left: 10%; margin-right: 10%">
          <div class="col-7">
            <p><b>Type:</b> {{ helper.displayWithCamelCase(type) }}</p>
            <b>Abilities:</b>
          ...
          <div class="col-5">
            <p><b>Color:</b> {{ helper.displayWithCamelCase(color) }}</p>
            <p><b>Shape:</b> {{ helper.displayWithCamelCase(shape) }}</p>
            <p><b>Habitat:</b> {{ helper.displayWithCamelCase(habitat) }}</p>
            <p><b>Generation:</b> {{ helper.displayWithCamelCase(generation) }}</p>
            <p><b>Growth rate:</b> {{ helper.displayWithCamelCase(growth_rate) }}</p>
          </div>
        </div>
      </div>
    </div>
    ...
    fetchAllPokemon(flag) {
      ...
      fetch(
        ...
        .then((allpokemon) => {
          ...
          allpokemon.results.map((el) => {
            this.fetchPokemon(el.name);
          });
        });
    },
    get_id(pokemon) {
