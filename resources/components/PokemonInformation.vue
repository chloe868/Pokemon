    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-footer">
          <button type="button" class="btn btn-danger" data-toggle="tooltip" data-placement="top" title="Add to hate" @click="addHateLikeFav('hate')">
            <font-awesome-icon icon="fa-solid fa-thumbs-down" />
          </button>
          <button type="button" class="btn btn-primary" data-toggle="tooltip" data-placement="top" title="Add to like" @click="addHateLikeFav('like')">
            <font-awesome-icon icon="fa-solid fa-thumbs-up" />
          </button>
          <button type="button" class="btn btn-warning" data-toggle="tooltip" data-placement="top" title="Add to favorite" @click="addHateLikeFav('fave')">
...
export default {
  ...
  methods: {
    ...
    addHateLikeFav(status) {
      ...
      this.$axios.post(AUTH.url + 'addPokemon', params, AUTH.config)
        .then(response => {
    ...
    fetchSpecificPokemon(name) {
      fetch(`https://pokeapi.co/api/v2/pokemon/${name}/`)
        ...
        .then((details) => {
          ...
          this.type = details.types[0].type.name.charAt(0).toUpperCase() + details.types[0].type.name.slice(1);
